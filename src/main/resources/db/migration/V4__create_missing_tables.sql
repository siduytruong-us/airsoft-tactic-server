-- ============================================================
-- V4: Create all missing tables (safe - uses IF NOT EXISTS)
-- ============================================================
...
-- Drop old game_modes if it has wrong schema (int4 id), then recreate
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'game_modes'
          AND column_name = 'id'
...
CREATE INDEX IF NOT EXISTS idx_events_field       ON events(field_id);
...
CREATE INDEX IF NOT EXISTS idx_matches_field  ON game_matches(field_id);
CREATE INDEX IF NOT EXISTS idx_matches_status ON game_matches(status);
...
-- FK winning_team (after teams table exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'fk_winning_team'
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_hits_match_user ON hit_events(match_id, user_id);
